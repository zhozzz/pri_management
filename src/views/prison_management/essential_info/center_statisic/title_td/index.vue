 ...
 * @LastEditors: zhuhao
 * @LastEditTime: 2020-04-25 20:19:57
 -->
<template>
  <td :rowspan="row" :colspan="col">
    ...
    >
      ...
      <div class="td_box column" slot="reference">
        <div>
          <el-tooltip placement="top" :disabled="dataErrorFlag === true" effect="light">
            <div slot="content">{{msgInfo.length > 0 ? msgInfo[0] : ''}}<br/>{{msgInfo.length > 1 ? msgInfo[1] : ''}}</div>
            <span class="title_name" v-bind:class="{ red: !dataErrorFlag }">{{titleName}}</span>
          </el-tooltip>
          <i class="el-icon-warning-outline"></i>
        </div>
        <!-- <div style="color:red" v-if="!dataErrorFlag">数据异常!</div> -->
      </div>
    </el-popover>
...
import { editDesc } from '../server.js'
...
export default {
  ...
  methods: {
    ...
    subDesc() {
      let params = {
        description: this.attentions,
        name: this.titleTip.name,
        id: this.titleTip.id
      }
    ...
    chancl() {
      ...
      this.attentions = this.titleTip.description
    }
  },
  watch: {
    titleTip: {
