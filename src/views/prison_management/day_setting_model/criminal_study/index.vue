 ...
 * @LastEditors: zhuhao
 * @LastEditTime: 2020-03-25 16:05:10
 -->
<template>
  <dept-layout @input="nodeClick" v-model="prisonId" :backData="true">
    <menuTree :menuList="['监狱管理','5+1+1模式执行','罪犯学习管理']">
      <tab slot="heade_btn" right name="罪犯学习管理" :pathName="'criminal_study_statistics'"></tab>
      <table-view
        :tableData="tableData"
        noIndex
      ...
      >
        <div slot="searchCondition">
          <el-form ref="form" class="yn_form" inline :model="queryParams" label-width="7rem">
            ...
            <el-form-item label="学习内容">
              <el-input class="yn_ipt" v-model="queryParams.studyTitle"></el-input>
            </el-form-item>
            ...
            <el-form-item label="学习类型">
              <el-select v-model="queryParams.studyType" class="yn_ipt" placeholder="请选择学习类型">
                <el-option
        ...
        <template slot="table-column">
          ...
          <el-table-column show-overflow-tooltip align="center" prop="studyTitle" label="学习内容" />
          <el-table-column show-overflow-tooltip align="center" prop="studyTypeName" label="学习类型" />
          ...
          <el-table-column show-overflow-tooltip align="center" label="是否学习日">
            <template slot-scope="{row}">
              <span v-if="row.isStudy===0">否</span>
              <span v-else-if="row.isStudy===1">否</span>
              <span v-else-if="row.isStudy===2">是</span>
              <span v-else>暂未配置</span>
            </template>
          </el-table-column>
          <el-table-column show-overflow-tooltip align="center" prop="studyPlace" label="学习地点" />
          ...
          <el-table-column align="center" prop="isOverTime" label="是否延时">
            <template slot-scope="{row}">
          ...
          <el-table-column show-overflow-tooltip align="center" label="操作" min-width="300">
            <template slot-scope="{row}">
              <div class="btn_box_inTable">
                ...
                <el-button class="yn_text_btn" type="text" @click="check(row)">查看</el-button>
                <el-button
                  class="yn_text_btn"
                  v-if="userPrisonId!=1"
                  type="text"
      ...
      <transfer-update
        ...
        :submitAPI="'/study/updateCriAndPol'"
        :params="updateParams"
        ...
        >
          <!-- <template slot="colum_btns">
            <el-table-column align="center" label="添加">
              <template slot-scope="{row}">
                <el-button type="text" class="yn_text_btn" @click="addNewGroup(row)">添加</el-button>
              </template>
            </el-table-column>
          </template>-->
        </criminalGroupInfo>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      this.axios
        .post(this._global.businessUrl + '/study/export', queryParams, {
          responseType: 'blob'
        })
        .then(msg => {
          ...
          let fileName =
            '罪犯学习管理-' + this.formatDate(new Date(), 'yyyy-MM-dd hh:mm:ss')
          a.href = url
    ...
    queryInfo() {
      ...
      this.axios
        .post(_global.businessUrl + '/study/selAll', this.queryParams)
        .then(
          res => {
    ...
    edit(rows) {
      ...
      this.editList.studyTime = [rows.beginTime, rows.endTime]
    ...
    del(row) {
      ...
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        .then(() => {
          this.axios
            .post(_global.businessUrl + '/study/delete', param)
            .then(res => {
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    updateWorkWarn(id, flag) {
      this.$confirm('确认修改预警状态?', '提示', {
        ...
        this.axios
          .get(_global.businessUrl + '/study/updateWarn/' + id + '/' + flag)
          .then(res => {
    ...
    completeAll(row) {
      this.$confirm('是否结束?', '提示', {
        ...
        .then(() => {
          this.axios
            .put(_global.businessUrl + '/study/oneStudyComplete/' + row)
            .then(res => {
              this.$message({
                type: 'success',
                message: res
