 ...
 * @LastEditors: zhuhao
 * @LastEditTime: 2020-03-25 15:07:09
 -->
<template>
  <menuTree v-loading="chargeLoading" :menuList="['监狱管理','监狱动态','监狱重大活动']">
    <tab slot="heade_btn" right name="编辑重大活动" :pathName="'major_activities_statistics'" back></tab>
    ...
    >
      <div>
        ...
        >
          <el-row :gutter="10">
            <el-col :span="10" :offset="1" label-width="8rem">
  ...
  methods: {
    ...
    getDetails() {
      ...
      checkQuery({ eventId: this.$route.query.eventId, flag: 1 }).then(res => {
        ...
        // 图片
        this.fileIdList = res.eventMainInfo.eventImagePath.split(',')

        if (res.eventMainInfo.imagePath) {
          this.fileList = res.eventMainInfo.imagePath.map(item => {
            return {
              name: item.file_name,
              url: item.file_path,
              id: item.file_id
    ...
    submitUpload() {
      ...
      this.axios
        ...
        .then(res => {
          ...
          this.fileIdList.map((item, i) => {
            this.editParams.eventMainInfo.eventImagePath += item
            if (i !== res.length - 1) {
              this.editParams.eventMainInfo.eventImagePath += ','
            }
          })
