 ...
 * @LastEditors: zhuhao
 * @LastEditTime: 2019-12-05 11:40:09
 -->
<template>
            ...
            formatter: data => {
              ...
              if (this.isNumber) {
                ...
                this.chartData.map(item => {
                  if (item[data.seriesName]) {
                    total += item[data.seriesName]
                  }
                })
                str = data.data[data.seriesName]

                if (str !== 0) {
                  str =
                    str + '(占总数的' + ((str / total) * 100).toFixed(2) + '%)'
                } else {
                  str = ''
                }
              } else {
                if (data.data[data.seriesName] !== 0) {
                  str = data.data[data.seriesName] + '%'
                } else {
                  str = ''
                }
